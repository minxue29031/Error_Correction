...
class DataExtractor:
                ...
                if len(row) >= 2:
                    ...
                    if column_1_tokens:  # Check if the first column is not empty
                        if len(column_2_tokens) <= 4:
                            ...
                            column_1_start_pos = max(column_2_start_pos - 1, 0)
                            column_1_end_pos = min(column_2_end_pos + 5, len(column_1_tokens) - 1)
    ...
    def write_to_csv(self, column_1, column_2):
        with open(self.output_csv_file, 'w', newline='', encoding=self.csv_encoding) as file:
            ...
            writer.writerow(['fail_fragment', 'succ_fragment'])  # Write the header row
    ...
    def process_data(self):
        ...
        fail_fragments = [sentence_tokens for sentence_tokens in filtered_column_1]
        succ_fragments = [sentence_tokens[-4:] for sentence_tokens in filtered_column_2]
