...
class DataExtractor:
                ...
                if len(row) >= 2:
                    ...
                    if column_1_tokens:  # Check if the first column is not empty
                        if len(column_2_tokens) <= 4:
                            ...
                            column_1_start_pos = max(column_2_start_pos, 0)
    ...
    def process_data(self):
        ...
        for i, sentence in enumerate(column_1):
            if sentence:
                filtered_column_1.append(tuple(sentence)) 
                filtered_column_2.append(tuple(column_2[i]))  

        self.write_to_csv(filtered_column_1, filtered_column_2)

        print("Extracted data for column 1 (POSi-2 to POSj+2):")
        for i in range(len(filtered_column_1)):
            tokens = filtered_column_1[i]
            print(f"Sentence {i+1}: {' '.join(tokens)}")

        print("Extracted data for column 2 (last four tokens):")
        for i in range(len(filtered_column_2)):
            tokens = filtered_column_2[i]
            print(f"Sentence {i+1}: {' '.join(tokens)}")

        # Write unique data from CSV file to tok files
        unique_data = set(zip(filtered_column_1, filtered_column_2))
        fail_fragments = [list(sentence_tokens) for sentence_tokens, _ in unique_data]  
        succ_fragments = [list(sentence_tokens[-4:]) for _, sentence_tokens in unique_data]  
